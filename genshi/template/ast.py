...
if _ast_emulated:
    ...
    class ASTUpgrader(object):
        ...
        def visitModule(self, node):
            ...
            if node.doc:
                body = [_new(_ast.Expr, _new(_ast.Str, node.doc))] + body
            return _new(_ast.Module, body)
        
        def visitExpression(self, node):
            return _new(_ast.Expression, self.visit(node.node))
        ...
        def _extract_args(self, node):
            ...
            def _tup(t):
                if isinstance(t, str):
                    return _new(_ast.Name, t, _ast.Store())
                elif isinstance(t, tuple):
                    elts = [_tup(x) for x in t]
                    return _new(_ast.Tuple, elts, _ast.Store())
                else:
            ...
            for arg in tab:
                if isinstance(arg, str):
                    args.append(_new(_ast.Name, arg, _ast.Param()))
            ...
            return _new(_ast.arguments, args, vararg, kwarg, defaults)
        ...
        def visitFunction(self, node):
            ...
            if node.doc:
                body = [_new(_ast.Expr, _new(_ast.Str, node.doc))] + body
            return _new(_ast.FunctionDef, node.name, args, body, decorators)
        ...
        def visitClass(self, node):
            ...
            if node.doc:
                body = [_new(_ast.Expr, _new(_ast.Str, node.doc))] + body
            return _new(_ast.ClassDef, node.name, bases, body)

        def visitReturn(self, node):
            return _new(_ast.Return, self.visit(node.value))

        def visitAssign(self, node):
            #self.name_types.append(_ast.Store)
            targets = [self.visit(t) for t in node.nodes]
            #self.name_types.pop()
            return _new(_ast.Assign, targets, self.visit(node.expr))
        ...
        def visitAugAssign(self, node):
            ...
            return _new(_ast.AugAssign, target, op, self.visit(node.expr))

        def _visitPrint(nl):
            def _visit(self, node):
                values = [self.visit(v) for v in node.nodes]
                return _new(_ast.Print, self.visit(node.dest), values, nl)
            return _visit
        ...
        def visitFor(self, node):
            return _new(_ast.For, self.visit(node.assign), self.visit(node.list),
                            self.visit(node.body), self.visit(node.else_))

        def visitWhile(self, node):
            return _new(_ast.While, self.visit(node.test), self.visit(node.body),
        ...
        def visitIf(self, node):
            def _level(tests, else_):
                ...
                return _new(_ast.If, test, body, orelse)
            return _level(node.tests, node.else_)

        # def visitWith

        def visitRaise(self, node):
            return _new(_ast.Raise, self.visit(node.expr1),
        ...
        def visitTryExcept(self, node):
            ...
            for type, name, body in node.handlers:
                handlers.append(_new(_ast.excepthandler, self.visit(type), 
                                self.visit(name), self.visit(body)))
            return _new(_ast.TryExcept, self.visit(node.body),
                            handlers, self.visit(node.else_))

        def visitTryFinally(self, node):
            return _new(_ast.TryFinally, self.visit(node.body),
                            self.visit(node.final))

        def visitAssert(self, node):
            return _new(_ast.Assert, self.visit(node.test), self.visit(node.fail))

        def visitImport(self, node):
            names = [_new(_ast.alias, n[0], n[1]) for n in node.names]
            return _new(_ast.Import, names)

        def visitFrom(self, node):
            names = [_new(_ast.alias, n[0], n[1]) for n in node.names]
            return _new(_ast.ImportFrom, node.modname, names, node.level)

        def visitExec(self, node):
            return _new(_ast.Exec, self.visit(node.expr),
                            self.visit(node.locals), self.visit(node.globals))

        def visitGlobal(self, node):
            return _new(_ast.Global, node.names[:])

        def visitDiscard(self, node):
            return _new(_ast.Expr, self.visit(node.expr))
        ...
        def _visitBinOperator(opcls):
            def _visit(self, node):
                return _new(_ast.BinOp, self.visit(node.left), 
        ...
        def _visitBitOperator(opcls):
            def _visit(self, node):
                ...
                def _make(nodes):
                    ...
                    return _new(_ast.BinOp, left, opcls(), right, lineno=lineno)
        ...
        def _visitUnaryOperator(opcls):
            def _visit(self, node):
                return _new(_ast.UnaryOp, opcls(), self.visit(node.expr))
            return _visit
        ...
        def _visitBoolOperator(opcls):
            def _visit(self, node):
                ...
                return _new(_ast.BoolOp, opcls(), values)
            return _visit
        ...
        def visitCompare(self, node):
            ...
            return _new(_ast.Compare, left, ops, comparators,
                            lineno = node.lineno)

        def visitLambda(self, node):
            args = self._extract_args(node)
            body = self.visit(node.code)
            return _new(_ast.Lambda, args, body)

        def visitIfExp(self, node):
            return _new(_ast.IfExp, self.visit(node.test), self.visit(node.then),
                            self.visit(node.else_))

        def visitDict(self, node):
            keys = [self.visit(x[0]) for x in node.items]
            values = [self.visit(x[1]) for x in node.items]
            return _new(_ast.Dict, keys, values)

        def visitListComp(self, node):
            generators = [self.visit(q) for q in node.quals]
            return _new(_ast.ListComp, self.visit(node.expr), generators)

        def visitGenExprInner(self, node):
            generators = [self.visit(q) for q in node.quals]
            return _new(_ast.GeneratorExp, self.visit(node.expr), generators)
        ...
        def visitGenExprFor(self, node):
            ...
            return _new(_ast.comprehension, self.visit(node.assign),
        ...
        def visitYield(self, node):
            return _new(_ast.Yield, self.visit(node.value))
        ...
        def visitCallFunc(self, node):
            ...
            for arg in node.args:
                if isinstance(arg, compiler.ast.Keyword):
                    keywords.append(_new(_ast.keyword, arg.name, 
                                            self.visit(arg.expr)))
                else:
                    args.append(self.visit(arg))
            return _new(_ast.Call, self.visit(node.node), args, keywords,
                        self.visit(node.star_args), self.visit(node.dstar_args))

        def visitBackquote(self, node):
            return _new(_ast.Repr, self.visit(node.expr))
        ...
        def visitConst(self, node):
            if node.value is None:
                ...
                return _new(_ast.Str, node.value)
            else:
                return _new(_ast.Num, node.value)

        def visitName(self, node):
            return _new(_ast.Name, node.name, _ast.Load())

        def visitGetattr(self, node):
            return _new(_ast.Attribute, self.visit(node.expr), node.attrname,
                            _ast.Load())

        def visitTuple(self, node):
            nodes = [self.visit(n) for n in node.nodes]
            return _new(_ast.Tuple, nodes, _ast.Load())

        def visitList(self, node):
            nodes = [self.visit(n) for n in node.nodes]
            return _new(_ast.List, nodes, _ast.Load())
        ...
        def visitAssName(self, node):
            ...
            return _new(_ast.Name, node.name, ctx)

        def visitAssAttr(self, node):
            self.out_flags = node.flags
            ctx = self.get_ctx(node.flags)
            return _new(_ast.Attribute, self.visit(node.expr), 
                                node.attrname, ctx)
        ...
        def visitSlice(self, node):
            ...
            ctx = self.get_ctx(node.flags)
            self.out_flags = node.flags
            return _new(_ast.Subscript, self.visit(node.expr),
                        _new(_ast.Slice, lower, upper, None), ctx)
        ...
        def visitSubscript(self, node):
            ...
            for sub in subs:
                if isinstance(sub, advanced):
                    ...
                    slices.append(_new(_ast.Index, sub))
            if len(slices) == 1:
                slice = slices[0]
            elif nonindex:
                slice = _new(_ast.ExtSlice, slices)
            else:
                slice = _new(_ast.Tuple, slices, _ast.Load())

            self.out_flags = node.flags
            return _new(_ast.Subscript, self.visit(node.expr), slice, ctx)

        def visitSliceobj(self, node):
            a = node.nodes + [None]*(3 - len(node.nodes))
            a = map(self.visit, a)
            return _new(_ast.Slice, a[0], a[1], a[2])

        def visitEllipsis(self, node):
            return _new(_ast.Ellipsis)
        ...
        def visitStmt(self, node):
            def _check_del(n):
                ...
                if isinstance(n, dcls) and isinstance(n.ctx, _ast.Del):
                    ...
                    return _new(_ast.Delete, n.elts)
                else:
                    return n
            def _keep(n):
